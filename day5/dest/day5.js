...
const getDataInput = () => {
    let crateArrangementInput = (0, fs_1.readFileSync)('./input.txt', { encoding: "utf-8" }).split('\n');
    return crateArrangementInput;
};
const orderOfTheCrateArrangement = () => {
    const crateArrangementInput = getDataInput();
    const moves = [];
    ...
    crateArrangementInput.map(column => {
        ...
        if (column.split(' ')[0] == 'move') {
            ...
            for (let i = 1; i < column.length; i++) {
                ...
                index += 4;
                const row2 = column[index];
                index += 4;
                const row3 = column[index];
                index += 4;
                const row4 = column[index];
                index += 4;
                const row5 = column[index];
                index += 4;
                const row6 = column[index];
                index += 4;
                const row7 = column[index];
                index += 4;
                const row8 = column[index];
                index += 4;
                const row9 = column[index];
    ...
    const totalRows = [row1, row2, row3, row4, row5, row6, row7, row8, row9,];
    return totalRows;
};
const movesOfTheCrates = () => {
    ...
    for (let i = 0; i < allTheNumsFromMoves.length; i += 3) {
        let rows = allTheNumsFromMoves.slice(i, i + 3);
...
const funPartOne = () => {
    ...
    for (let i = 0; i < (numbersPerMovementRow.length); i++) {
        ...
        const amountToMove = +numbersPerMovementRow[i][0];
        // The second index is from which row the crates come
        const fromRow = +numbersPerMovementRow[i][1];
        // The third index is the row to which the crates go
        const toRow = +numbersPerMovementRow[i][2];
...
const answer = () => {
    ...
    let topValuesPartOne = '';
    let topValuesPartTwo = '';
